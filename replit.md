...
## Phase 4 — Ops Interface Expansion (April 2026)

Unifies every internal-team workflow into a single password-gated `/ops` shell and adds three new operational primitives: custom-T&C PDF moderation, referral-payout tracking, and per-user push-notification debugging. Defers dispute/refund schema and an audit log.

- **Schema** (`shared/schema.ts`): `users` gains `suspendedAt`, `suspendedReason`, `suspendedBy` (suspension is recorded but not yet enforced at login/join — by design, so accidental suspensions are reversible). `groups` gains the same triplet plus `customTcStatus` ∈ {`pending`, `approved`, `rejected`}, `customTcReviewNote`, `customTcReviewedAt`, `customTcReviewedBy` for the custom-T&C PDF moderation queue. `referrals` gains `paidAt` + `paidBy` so completed referrals can be marked as paid out by the team. `insertReferralSchema` omits `paidAt`/`paidBy`. Schema sync was applied via direct `ALTER TABLE` SQL (the Drizzle interactive prompt could not accept stdin in the sandbox); existing custom-mode groups were back-filled with `customTcStatus='approved'`.
- **Storage** (`server/storage.ts`, DbStorage prototype-assignment + interface declaration-merging at end of file): `searchUsers(q, limit)` (prefix match on name / phone / referral code, plus exact-id), `getUserDetail(userId)` (user record + admin/member group memberships + contributions count + push-subscription count), `searchGroups(q, limit)`, `getGroupDetail(groupId)` (group record + admin + member/project counts + contribution totals + recent members), `suspendUser`/`unsuspendUser`, `suspendGroup`/`unsuspendGroup`, `listPendingCustomTerms()` (groups where `tcMode='custom' AND customTcStatus='pending'`), `reviewCustomTerms(groupId, decision, note, byUserId)`, `markReferralPaid(referralId, byUserId)` (rejects unless `status='complete'`), `unmarkReferralPaid(referralId)`. `setGroupTerms` is patched so a custom-PDF upload sets `customTcStatus='pending'` automatically.
- **Routes** (`server/routes.ts`, all gated by a small `requireOpsAuth` helper that accepts the password via `x-ops-password` header, `?password=` query, or JSON body): `GET /api/ops/users/search`, `GET /api/ops/users/:id`, `POST /api/ops/users/:id/suspend`, `POST /api/ops/users/:id/unsuspend`; the matching `/api/ops/groups/...` quintuple; `GET /api/ops/custom-terms/pending` + `POST /api/ops/custom-terms/:groupId/review`; `POST /api/ops/referrals/:id/mark-paid` + `/unmark-paid`; `POST /api/ops/push-test` (sends a one-off web-push to a specified user, returns per-subscription delivery results); `GET /api/ops/admin-kyc/pending` and `POST /api/ops/admin-kyc/:userId/review` mirror the existing super-admin KYC routes but accept the ops password instead of `KONTRIB_SUPERADMIN_USER_ID`, so any team member with the password can review.
- **Frontend shell** (`client/src/pages/ops.tsx`): single password lock screen → unified header with section pill-bar nav (Overview, Admin KYC, Custom T&C, Verified Ajo, Users, Groups, Payments, Referrals, Push test). Each pill shows a live queue-size badge fetched from the shell. The Overview pane summarises rewards owed, pending proofs, completed referrals, and links into each queue. Existing Verified-Ajo decisioning, payments listing, and referrals listing are preserved; the referrals tab is extended with an "Owed" filter, total-owed banner, and per-row **Mark as paid / undo** actions wired to the new endpoints.
- **Shared helpers** (`client/src/components/ops/ops-shared.tsx`): tiny `opsFetch` wrapper that auto-attaches the password (stored in `sessionStorage` under `kontrib_ops_pass`) as both a query param and an `x-ops-password` header, plus shared `formatDateTime`, `formatNgn`, `StatusPill` so the panels stay terse.
- **New panels**:
  - `users-panel.tsx` — search by name / phone / id / referral code, infinite-row list, click opens a side `Sheet` with the full record (role, KYC status, push subscription count, lifetime contributed, group memberships) plus suspend / unsuspend (suspend requires a 3+ char reason).
  - `groups-panel.tsx` — same shape: search by name / id / slug / registration link, side-drawer detail with member/project/contribution stats and T&C-mode info, suspend / unsuspend with reason.
  - `custom-terms-panel.tsx` — moderation queue listing each pending PDF with a direct preview link, optional approval note (required for rejection), and Approve/Reject buttons.
  - `kyc-panel.tsx` — admin-KYC queue identical to `/kontrib/kyc-review` but powered by the ops-password mirrored endpoints, so every shell user can clear it without needing the super-admin user id.
  - `push-test-panel.tsx` — pick a user via search, edit title/body/click-URL, send. Renders the per-subscription delivery results so the team can confirm which devices are reachable (and the server prunes any 404/410 endpoints).
- **Actor identification**: each mutating route accepts an optional `actorId`; the shell forwards `getCurrentUser().id` if the ops member is also signed in, otherwise the literal string `"ops"`. Stored on `suspendedBy` / `customTcReviewedBy` / `paidBy` columns for future audit-log work.

Smoke-tested: `users/search`, `groups/search`, `users/:id`, `custom-terms/pending`, `admin-kyc/pending`, `push-test`, and `referrals/:id/mark-paid` all respond as expected (push-test correctly reports "no active push subscriptions" for users without devices; mark-paid correctly rejects unknown referral ids with HTTP 400).
