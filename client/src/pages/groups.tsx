            ...
            {bucketedGroups.map((bucket) => {
              ...
              return (
                <section key={bucket.type} data-testid={`bucket-${bucket.type}`}>
                  <div className={`flex items-center gap-2.5 px-3 py-2 mb-2 rounded-xl border ${bucket.meta.accentBg}`}>
                    <div className={`w-7 h-7 rounded-lg bg-white flex items-center justify-center ${bucket.meta.accentText} shadow-sm`}>
                      <SectionIcon className="h-4 w-4" />
                    </div>
                    <h2 className="flex-1 text-sm font-bold text-gray-900">
                      {bucket.meta.label}
                    </h2>
                    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full ${bucket.meta.badgeBg} ${bucket.meta.accentText}`}>
                      {bucket.items.length}
                    </span>
                  </div>
